...
console.log("🚀 Text Area Selector extension loaded!");
document.addEventListener('keydown', function(event) {
    // Debug: Log all key combinations with Ctrl+Alt
    if (event.ctrlKey && event.altKey) {
        console.log('🔧 DEBUG: Ctrl+Alt+' + event.key + ' detected, shift=' + event.shiftKey);
    }
...
debugger;
    ...
    // Check for Ctrl+Shift+Alt+Q combination
    if (event.ctrlKey && event.shiftKey && event.altKey && event.key.toLowerCase() === 'q') {
        event.preventDefault();

        console.log('Ctrl+Shift+Alt+Q pressed - sending clipboard signal to server');

        // Just send POST request to trigger clipboard reading on server
        sendClipboardSignal();
    }
});
...
function sendClipboardSignal() {
    try {
        console.log('📋 Sending clipboard signal to local server...');

        fetch('http://localhost:9999?clipboard=true', {
            method: 'POST',
            headers: {
                'Content-Type': 'text/plain',
            },
            body: 'clipboard_signal'
        })
        .then(response => {
            if (response.ok) {
                console.log('✅ Successfully sent clipboard signal to server');
            } else {
                console.error('❌ Server responded with error:', response.status, response.statusText);
            }
        })
        .catch(error => {
            console.error('❌ Failed to send clipboard signal:', error);
            console.log('💡 Make sure your server is running on http://localhost:9999');
        });

    } catch (error) {
        console.error('❌ Error sending clipboard signal:', error);
    }
}
