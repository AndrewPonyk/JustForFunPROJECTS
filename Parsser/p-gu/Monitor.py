...
class FileMonitorApp:
                    ...
                    if len(parts) == 2:
                        ...
                        if os.path.exists(target_filepath):
                            ...
                            if search_query:
                                ...
                                # Print the actual content that will be sent
                                self.add_log(f'Content of {target_filepath}:\n{search_query}')
                                try:
                                    # Perform the OpenAI call
                                    search_results = ask_openai_question(search_query)
                                    self.clear_log()
                                    self.add_log(f'{search_results}\n----------------------')
                                    # Scroll to the beginning of the text area after the UI is idle
                            ...
                            f.truncate()
                            f.write(new_content)
                            # Do not auto-scroll this maintenance log to keep the view at the top of the answer
                            self.add_log(f'Updated {os.path.basename(file_path)} to: {new_content}', auto_scroll=False)
                        else:
    ...
    def add_log(self, message, auto_scroll=True):
    ...
    def get_clipboard_text(self, timeout_seconds: float = 2.0) -> str:
        """Safely get clipboard text from the Tk main thread.
        Returns empty string if clipboard is empty or on timeout.
        """
        ...
        def fetch():
            try:
                # clipboard_get can raise if clipboard doesn't contain text
                result["text"] = self.root.clipboard_get()
            except Exception as e:
                result["error"] = str(e)
            finally:
                event.set()

        # Schedule on Tk main loop thread
...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        # Parse query first to see if clipboard mode is requested
        parsed = urlparse(self.path)
        query = parse_qs(parsed.query)
        use_clipboard = str(query.get('clipboard', ['false'])[0]).lower() == 'true'

        content_length = int(self.headers.get('Content-Length', 0))
        if content_length == 0 and not use_clipboard:
        ...
        try:
            ...
            # Choose data source: clipboard vs request body
            if use_clipboard:
                try:
                    clip_text = app.get_clipboard_text()
                    data_to_write = (clip_text or '').encode('utf-8')
                    app.add_log(f"Using clipboard content ({len(data_to_write)} bytes) due to clipboard=true")
                except Exception as e:
                    # Fallback to request body on error
                    app.add_log(f"Clipboard read failed ({e}); falling back to request body")
                    data_to_write = post_data
            else:
                data_to_write = post_data

            # Write the chosen data to the file
